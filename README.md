...
- RAG, via qdrant Vector DB + ollama models
- fastapi as backend
...
3. ingestion ([fastapi]) for ingesting new data into the VectorDB
4. query ([fastapi]) for querying the vectorDB and providing the interaction with the LLM
5. gateqay ([fastapi]) as an entrypoint to interact with the `query` and `ingestion` service
